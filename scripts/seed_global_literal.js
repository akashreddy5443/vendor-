...
async function seedGlobal() {
    try {
        ...
        console.log("Cleaning (Literal Mode)...");
        ...
        for (const c of categories) {
            // Check
            const checkSql = "SELECT id FROM categories WHERE slug = " + escapeLiteral(c.slug);
            let res = await client.query(checkSql);

            if (res.rows.length === 0) {
                console.log(`Creating: ${c.name}`);
                const insertSql = `INSERT INTO categories (name, slug, description) VALUES (${escapeLiteral(c.name)}, ${escapeLiteral(c.slug)}, 'Default') RETURNING id`;
        ...
        for (const p of products) {
            ...
            const insertSql = `
                ...
                VALUES (
                    ${escapeLiteral(p.title)}, 
                    ${escapeLiteral(p.slug)}, 
                    ${escapeLiteral(p.description)}, 
                    ${p.price}, 
                    ${p.stock}, 
                    'active', 
                    ${escapeLiteral(catId)}
                ...
                RETURNING id
            `;
            const pRes = await client.query(insertSql);
            insertedIds.push(pRes.rows[0].id);

            const imgSql = `
                INSERT INTO product_images (product_id, cloudinary_url, media_type, is_primary)
                VALUES (
                    ${escapeLiteral(pRes.rows[0].id)},
                    ${escapeLiteral(p.image)},
                    'image',
                    true
                )
            `;
        ...
        if (insertedIds.length >= 3) {
            const ids = insertedIds.slice(0, 3);
            const jsonStr = JSON.stringify({ productIds: ids });
            const featSql = `
                INSERT INTO homepage_sections (section_type, title, subtitle, content_json, is_active)
                VALUES ('featured', 'Featured Gear', 'Brief', ${escapeLiteral(jsonStr)}, true)
                ON CONFLICT (section_type)
                DO UPDATE SET content_json = ${escapeLiteral(jsonStr)}, is_active = true
            `;
            await client.query(featSql);
            console.log('✅ Updated Featured.');
        }
