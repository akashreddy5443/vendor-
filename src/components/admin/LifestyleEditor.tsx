...
export function LifestyleEditor({ initialItems, initialSubtitle, initialTitle }: { initialItems: Item[], initialSubtitle?: string, initialTitle?: string }) {
    ...
    const [badgeText, setBadgeText] = useState((initialItems as any)?.badgeText || 'Featured')
    const [btnText, setBtnText] = useState((initialItems as any)?.btnText || 'Explore Now →')
    const [microText, setMicroText] = useState((initialItems as any)?.microText || 'Explore Collection')
    ...
    const handleSubmit = async (formData: FormData) => {
        // We pack the extra text fields into the items array or a separate object?
        // Let's pack them into a separate config object or just append to formData
        // But the action expects 'items' to be the array.
        // Let's pass them as separate fields to action, OR wrapper object.
        // Looking at acton: items = JSON.parse(formData.get('items'))
        // Let's modify the action separately.
        formData.append('items', JSON.stringify(items))
        formData.append('subtitle', subtitle)
        formData.append('title', title)
        formData.append('badgeText', badgeText)
        formData.append('btnText', btnText)
        formData.append('microText', microText)
            ...
            <form action={handleSubmit} className="space-y-8">
                ...
                <div className="bg-gradient-to-br from-indigo-50 to-blue-50 p-6 rounded-xl border border-indigo-100 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-indigo-100 pt-4">
                        <label>
                            <span className="text-xs font-bold text-slate-700 mb-1 block">Badge Text (First Card)</span>
                            <input value={badgeText} onChange={e => setBadgeText(e.target.value)} className="w-full rounded-lg border border-slate-200 p-2 text-sm" />
                        </label>
                        <label>
                            <span className="text-xs font-bold text-slate-700 mb-1 block">Hover Button Text</span>
                            <input value={btnText} onChange={e => setBtnText(e.target.value)} className="w-full rounded-lg border border-slate-200 p-2 text-sm" />
                        </label>
                        <label>
                            <span className="text-xs font-bold text-slate-700 mb-1 block">Link Label</span>
                            <input value={microText} onChange={e => setMicroText(e.target.value)} className="w-full rounded-lg border border-slate-200 p-2 text-sm" />
                        </label>
                    </div>
                </div>
